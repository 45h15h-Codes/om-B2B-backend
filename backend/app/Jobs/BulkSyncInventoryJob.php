...
class BulkSyncInventoryJob implements ShouldQueue
{
    ...
    public function handle(InventoryManager $inventoryManager)
    {
        foreach ($this->productIds as $id) {
            ...
            if ($product) {
                try {
                    $inventoryManager->sync($product, $this->adminId, $this->remarks, $this->ipAddress);
                } catch (\Throwable $e) {
                    \Illuminate\Support\Facades\Log::error("Bulk Sync Error for product ID {$id}: " . $e->getMessage());
                }
            }
        }
    }
